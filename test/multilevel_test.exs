defmodule NebulexLocalDistributedAdapter.MultilevelTest do
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      if Process.alive?(pid), do: Cache.Standalone.stop(pid)
    end)
  ...
  describe "entry:" do
    ...
    # needs fixing in base Multilevel adapter?
    @tag :skip
    test "get_all/2 replicates to lower levels", %{cache: cache} do
      assert cache.put_all([a: 1, c: 3], level: 3) == :ok
      assert cache.get_all([:a, :b, :c], level: 1) == %{}
      assert cache.get_all([:a, :b, :c], level: 3) == %{a: 1, c: 3}
      assert cache.get_all([:a, :b, :c]) == %{a: 1, c: 3}
      assert cache.get_all([:a, :b, :c], level: 1) == %{a: 1, c: 3}
    end
